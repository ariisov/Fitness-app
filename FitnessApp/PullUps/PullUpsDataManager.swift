...
class PullUpsDataManager {
    
    private var count = 0

    private var counterNumberOfPullUps = 0

    func obtainNumberOfPullUps() -> String {
        counterNumberOfPullUps += 1
        return String(counterNumberOfPullUps)
    }
            ...
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    ...
                    db.collection("users").document(Auth.auth().currentUser!.uid).updateData([
                        ...
                        "totalKcal": String(dataNumberOfKcal + self.counterNumberOfPullUps*2),
